...
carDelete31::carDelete31(wisdom*w,QWidget*parent)
    :QDialog(parent){
    ...
    setWindowTitle("编辑车位信息");
    label1=new QLabel(tr("请输入车主姓名："));
    label2=new QLabel(tr("姓名"));
    label3=new QLabel(tr("车位"));
    label4=new QLabel(tr("车牌号"));
    label5=new QLabel(tr("联系方式"));
    line1=new QLineEdit;
    line2=new QLineEdit;
    line3=new QLineEdit;
    line4=new QLineEdit;
...
void carDelete31::cancel(){

    this->close();
    wis->show();

}
void carDelete31::find(){
        ...
        while(!txtInput.atEnd()){
            ...
            if(jsonError.error==QJsonParseError::NoError){
                if(parseDoc.isObject()){
                    ...
                    if(jsonObj.contains("park")){
                        QJsonValue parkValue=jsonObj.take("park");
                        if(parkValue.isString()){
                            str2=parkValue.toString();
                            qDebug()<<str2;

                        }
                    }
                    if(jsonObj.contains("carNumber")){
                        QJsonValue carNumberValue=jsonObj.take("carNumber");
                        if(carNumberValue.isString()){
                            str3=carNumberValue.toString();
                            qDebug()<<str3;
                    ...
                    if(jsonObj.contains("telephone")){
                        ...
                        if(telephoneValue.isDouble()){
                            str4=telephoneValue.toString();
                            int n=telephoneValue.toInt();
                            str4=tr("%1").arg(n);
                            qDebug()<<str4;
...
void carDelete31::edit(){
    ...
    QFile file1("../wisdom/wisdom/car31.txt");
    ...
    while(!txt.atEnd()){
        ...
        if(text==str1){
            ...
            json.insert("park",QString(str2));
            json.insert("carNumber",QString(str3));
            json.insert("telephone",n4);
    ...
    QFile file4("../wisdom/wisdom/car31.txt");
...
void carDelete31::submit(){
    QString name=line2->text();
    QString park=line3->text();
    QString carNumber=line4->text();
    int telephone=line5->text().toInt();
    QJsonObject json;
...
void carDelete31::loadStyleSheet(const QString &sheetName)
{
